...
- [`messaging/reports.md`](docs/messaging/reports.md) — How to publish recurring reports — the ops digest, /report, audits — into a persistent, updatable channel document (a Slack canvas) instead of a new chat message each time.
...
- [`components/messaging.md`](specs-components/messaging.md) — Design contract for the provider-neutral messaging abstraction (Telegram/Slack/Discord/Matrix): the required core, the optional-capability tiers that degrade to text, and the report-surface contract for persistent updatable documents.
